...
def IngestDataProducts (app:App):
    ...
    dataCatalogo=GetDataSourceCatalogo(conn)
    createTableCatalogo(conn)
    InsertManyCatalogos(bd,dataCatalogo)
...
def GetDataSourceCatalogo(conn):
    pathData = "/workspaces/Practica3/proyecto/files/datafuente.xls"
    df = pd.read_excel(pathData, sheet_name="Orders")

    # Asegurar que las columnas existen
    print("Columnas disponibles en 'Orders':", df.columns)

    # Seleccionar las columnas correctas y eliminar duplicados
    df_catalogo = df[['Customer_ID', 'Customer_Name', 'Segment']].dropna().drop_duplicates()

    # Depuración: Verificar el contenido de df_catalogo
    print("Primeras filas de df_catalogo:", df_catalogo.head())

    # Convertir DataFrame a lista de tuplas para la inserción
    catalogo_tuples = [tuple(x) for x in df_catalogo.to_records(index=False)]

    return catalogo_tuples


def createTableCatalogo(conn:Connection):
    catalogo=Catalogo()
    catalogo.create_table(conn)

def InsertManyCatalogos(bd:Database,data):
    bd.insert_many('CATALOGO',['Customer_ID','Customer_Name','Segment'],data)    
